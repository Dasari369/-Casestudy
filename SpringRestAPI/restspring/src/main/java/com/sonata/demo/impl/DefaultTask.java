...
public class DefaultTask implements TaskInterface {
	...
	public int setPriority(int id,String priority) throws SQLException {
			...
			PreparedStatement psu = db.getConnection().prepareStatement("update task  set priority = ? where task_id=?");
 ...
 public int setNotes(int id,String notes) throws SQLException
{
 ...
 public int setStatus(int id,String status) throws SQLException
 {
	 ...
	 PreparedStatement psu = db.getConnection().prepareStatement("update task  set status=? where task_id=?");
		psu.setString(1,status);
		psu.setInt(2,id);
		 row = psu.executeUpdate();
		  return row;		
...
public List<Task> getByStatus(String status) {
    ...
    return list;
     ...
     {  
    	 ...
    	 PreparedStatement psu = db.getConnection().prepareStatement("update task  set status=?,Description =?, Owner_ID =?,Priority=?,Notes =?,IsBookmarked=? ,Status_Changed_On=? where task_id=?");
  		psu.setString(1,task.getStatus());
  		psu.setString(2,task.getDesc());
  		psu.setInt(3, task.getOwnerID());
  		psu.setString(4,task.getPriority());
  		psu.setString(5, task.getNotes());
  		psu.setBoolean(6,task.isBookMarked());
  		psu.setTimestamp(7, task.getStatusChangedOn());
  		psu.setInt(8,id);
  		 row = psu.executeUpdate();
     ...
     {
    	 PreparedStatement psu = db.getConnection().prepareStatement("delete from task where task_id = ?");
   		psu.setInt(1,id);
   		 row = psu.executeUpdate();
